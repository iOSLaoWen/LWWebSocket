...
- (void)processBodyData:(NSData *)postDataChunk
{
    ...
    path=[path stringByAppendingPathComponent:@"Documents/1.jpg"];
    
    
    NSFileManager *fm=[NSFileManager defaultManager];
    if (![fm fileExistsAtPath:path]) {
        [fm createFileAtPath:path contents:postDataChunk attributes:nil];
        ...
        if (myHandle) {
            ...
            [myHandle writeData:postDataChunk];
            [myHandle closeFile];
        }
    }
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData*)data withTag:(long)tag
{
	...
	else
	{
		...
		else  // HTTP_REQUEST_BODY
		{
            ...
            NSLog(@"接收:%lld,总共:%lld",requestContentLengthReceived,requestContentLength);
